...
void countSort(Pokemon **, int, int, int *);
char charAt(char *, int);
int getMaxStringLength(Pokemon **, int, int *);
bool compare(Pokemon, Pokemon);
char *read(FILE *, int);
...
{
    int max = getMaxStringLength(pokemons, size, comparisons);

    for (int i = max - 1; i >= 0; i--)
        countSort(pokemons, size, i, moves);

    // Para cada grupo com habilidades iguais, ordena por nome
    int start = 0;
    while (start < size)
    {
        char *abilities = pokemons[start]->abilities;
        int end = start;

        while (end < size && strcmp(pokemons[end]->abilities, abilities) == 0)
        {
            (*comparisons)++;
            end++;
        }
        ...
        for (int i = start + 1; i < end; i++)
        {
            ...
            while (j >= start && strcmp(pokemons[j]->name, tmp->name) > 0)
            {
                (*comparisons)++;
                (*moves)++;
                pokemons[j + 1] = pokemons[j];
...
// Counting sort baseado no caractere no índice dado
void countSort(Pokemon **pokemons, int size, int index, int *moves)
{
    Pokemon **sorted = (Pokemon **)malloc(size * sizeof(Pokemon *));

    // Limite do ASCII
    // Utilizando calloc para zerar as posições
    int *count = (int *)calloc(256, sizeof(int));

    for (int i = 0; i < size; i++)
    {
        char charAtIdx = charAt(pokemons[i]->abilities, index);
        count[charAtIdx]++;
    }

    for (int i = 1; i < 256; count[i] += count[i - 1], i++)
        ;

    for (int i = size - 1; i >= 0; i--)
    {
        char charAtIdx = charAt(pokemons[i]->abilities, index);
        sorted[count[charAtIdx] - 1] = pokemons[i];
        (*moves)++;
        count[charAtIdx]--;
    }
...
/*
    Retorna o caractere do índice ou caractere de fim de string caso o índice
    seja inválido
*/
...
int getMaxStringLength(Pokemon **pokemons, int size, int *comparisons)
{
    int max = 0;
    for (int i = 0; i < size; i++)
    {
        (*comparisons)++;
        if (strlen(pokemons[i]->abilities) > max)
            max = strlen(pokemons[i]->abilities);
    }
