...
int main()
{
    ...
    */
    ...
    file = fopen("842536_insercao.txt", "w");
...
void sort(Pokemon **pokemons, int size, int *comparisons, int *moves)
{
    for (int i = 1; i < size; i++)
    {
        ...
        if (j > 9)
        {
            if (compare(*tmp, *pokemons[9]) > 0)
                continue;

            pokemons[i] = pokemons[9];
            pokemons[9] = tmp;
            (*moves) += 2;
            j = 8;
        }

        while (j >= 0 && compare(*pokemons[j], *tmp) > 0)
        {
...
int compare(Pokemon pokemon1, Pokemon pokemon2)
{
    ...
    sscanf(pokemon1.captureDate, "%d/%d/%d", &d1, &m1, &y1);
    sscanf(pokemon2.captureDate, "%d/%d/%d", &d2, &m2, &y2);
