...
public class Counting 
{
    ...
    public static void main(String[] args) 
    {
        ...
        FileUtil.writeFile("842536_countingsort.txt",
            "842536\t" + comparisons + "\t" + moves + "\t" + 
                String.format("%.1f", delta) + "ms");
    }

    // Counting sort
    static void countingSort(Pokemon[] pokemons)
    {   
        int greatest = getGreatest(pokemons);

        int[] count = new int[greatest + 1];
        Pokemon[] ordered = new Pokemon[pokemons.length];

        for(int i = 0; i < count.length; count[i] = 0, i++);

        for(int i = 0; i < pokemons.length; 
            count[pokemons[i].getCaptureRate()]++, i++);

        for(int i = 1; i < count.length; count[i] += count[i - 1], i++);

        for(int i = pokemons.length - 1; i >= 0;
            ordered[count[pokemons[i].getCaptureRate()] - 1] = pokemons[i],
            count[pokemons[i].getCaptureRate()]--, moves++, i--);

        //Para cada grupo de captureRates iguais, ordena os pokemons por nome
        int start = 0;
        while(start < ordered.length)
        {
            int captureRate = ordered[start].getCaptureRate();
            int end = start;

            while(end < ordered.length &&
                ordered[end].getCaptureRate() == captureRate)
            {
                comparisons++;
                end++;
            }
            ...
            {
                Pokemon tmp = ordered[i];
                int j = i - 1;

                while(j >= start && ordered[j].getName().compareTo(tmp.getName()) > 0)
                {
                    comparisons++;
                    moves++;
        ...
        for(int i = 0; i < pokemons.length; pokemons[i] = ordered[i], moves++, i++);
    }

    // Retorna o maior valor do atributo captureRate
    static int getGreatest(Pokemon[] pokemons)
    {
        int greatest = 0;
        ...
        {
            comparisons++;
            if(pokemons[i].getCaptureRate() > greatest) 
                greatest = pokemons[i].getCaptureRate();
        }
