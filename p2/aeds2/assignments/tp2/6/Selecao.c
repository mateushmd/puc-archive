...
void selectionSort(Pokemon **, int, int *, int *);
void sort(Pokemon **, int, int, int *, int *);
int firstIndex(Pokemon **, int, int, int, int *);
...
int main()
{
    ...
    selectionSort(pokemons, count, &comparisons, &moves);
...
// Fachada do Selection Sort recursivo
void selectionSort(Pokemon **pokemons, int size, int *comparisons, int *moves)
{
    sort(pokemons, size, 0, comparisons, moves);
}

// Selection Sort recursivo
void sort(Pokemon **pokemons, int size, int i, int *comparisons, int *moves)
{
    if (i >= size - 1)
        return;

    int min = firstIndex(pokemons, size, i + 1, i, comparisons);

    (*moves) += 3;
    swap(pokemons[i], pokemons[min]);

    sort(pokemons, size, ++i, comparisons, moves);
}

// Função recursiva para encontrar o pokemon que vem antes em ordem alfabética
int firstIndex(Pokemon **pokemons, int size, int i, int min, int *comparisons)
{
    if (i >= size)
        return min;

    (*comparisons)++;
    if (strcmp(pokemons[min]->name, pokemons[i]->name) > 0)
        min = i;

    return firstIndex(pokemons, size, ++i, min, comparisons);
}
