...
void sort(Pokemon **pokemons, int size, int *comparisons, int *moves)
{
    for (int i = 0; i < size - 1; i++)
    {
        ...
        {
            (*comparisons)++;
            if (compare(*pokemons[j + 1], *pokemons[j]))
            {
...
/*
    Retorna TRUE se os critérios de comparação indicam que pokemon1 vem antes,
    FALSE caso contrário
*/
bool compare(Pokemon pokemon1, Pokemon pokemon2)
{
    int idDiff = pokemon1.id - pokemon2.id;

    if (idDiff != 0)
        return idDiff < 0;

    return strcmp(pokemon1.name, pokemon2.name) <= 0;
}
