...
bool binarySearch(Pokemon **, char *, int, int *);
char *read(FILE *, int);
...
int main()
{
    ...
    while (strncmp(line, "FIM", 3) != 0)
    {
        ...
        binarySearch(pokemons, line, count, &comparisons) ? printf("SIM\n") : printf("NAO\n");
    ...
    /*
        Calculando o tempo em milissegundos que a pesquisa binária levou para
        encontrar todos os pokemons
    */
    double delta = (double)(endTime - beginTime) / CLOCKS_PER_SEC * 1000;

    file = fopen("842536_binaria.txt", "w");
...
// Pesquisa binária
bool binarySearch(Pokemon **pokemons, char *target, int count, int *comparisons)
{
    int r = count - 1, l = 0, mid;

    while (l <= r)
    {
        mid = (r + l) / 2;

        int cmp = strcmp(pokemons[mid]->name, target);
        (*comparisons)++;
