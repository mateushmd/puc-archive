...
class HashTable
{
    ...
    int tableSize, overflowSize;

    public HashTable(int tableSize, int overflowSize)
    {
        this.tableSize = tableSize;
        this.overflowSize = overflowSize;
        
        pokedex = new Pokemon[tableSize + overflowSize];
    }
    ...
    private int hash(String pokemonName)
    {
        return ascii(pokemonName) % tableSize;
    }

    private int rehash(String pokemonName)
    {
        return (ascii(pokemonName) + 1) % tableSize;
    }

    public void insert(Pokemon pokemon)
    {
        int idx = hash(pokemon.getName());

        if(pokedex[idx] != null)
        {
            for(int i = 0; i < overflowSize; i++)
            {
                if(pokedex[i + tableSize] == null)
                {
                    pokedex[i + tableSize] = pokemon;
                    return;
                }
            }
        }
    ...
    public SearchResult search(String pokemonName)
    {
        ...
        int idx = hash(pokemonName);

        if(pokedex[idx] != null)
        {
            searchResult.comparisons++;

            if(pokedex[idx].getName().equals(pokemonName))
            {
        ...
        for(int i = 0; i < overflowSize; i++)
        {
            ...
            if(pokedex[i + tableSize].getName().equals(pokemonName))
            {
                searchResult.index = i + tableSize;
                searchResult.found = true;
                break;
            }
        }
...
public class Main 
{
    public static void main(String[] args) throws IOException
    {
        ...
        HashTable table = new HashTable(21, 9);
        ...
        while(!line.equals("FIM"))
        {
            ...
            table.insert(Pokemon.read(strPokemon));
            line = MyIO.readLine();
        }
        ...
        while(!line.equals("FIM"))
        {
            HashTable.SearchResult searchResult = table.search(line);
            MyIO.print("=> " + line + ": ");
            if(searchResult.index > -1) MyIO.print("(Posicao: " + searchResult.index + ") ");
            MyIO.println(searchResult.found ? "SIM" : "NAO");
        ...
        FileUtil.writeFile("842536_hashReserva.txt",
