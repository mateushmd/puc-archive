...
Stack *createStack()
{
    ...
    stack->size = 0;
    stack->pokemons = (Pokemon **)malloc(POKEDEX_SIZE * sizeof(Pokemon *));
    return stack;
}

void push(Stack *stack, Pokemon *pokemon)
{
    stack->pokemons[stack->size++] = pokemon;
}

Pokemon *pop(Stack *stack)
{
    return stack->pokemons[--stack->size];
}
